@extends('tenants.finance.Common.common')

@section('body')
    <div class="tab-content">
        <div class="block tab-pane" id="bank">           
            <div class="card" id="ordersTable">
                <div class="card-body">
                    <div class="flex items-center gap-3 mb-4">
                        <h6 class="text-15 grow">Bank</h6>
                        <div class="shrink-0">
                            <a href="#!" data-modal-target="addBank" type="button"
                        ...
                        <table class="w-full whitespace-nowrap">
                            <thead class="ltr:text-left rtl:text-right">
                                <tr>
                                    ...
                                    <th class="px-3.5 py-2.5 font-semibold border border-slate-200 dark:border-zink-500">Swift</th>
                            ...
                            <tbody class="">
                                @foreach($banks as $bank)
                                    <tr>
                                        ...
                                        <td class="px-3.5 py-2.5 border border-slate-200 dark:border-zink-500">{{$bank->name}}</td>
                                        <td class="px-3.5 py-2.5 border border-slate-200 dark:border-zink-500">{{$bank->swift}}</td>
                                        <td class="px-3.5 py-2.5 border border-slate-200 dark:border-zink-500">
                                            <div class="flex gap-2">
                                                <a href="#!" data-modal-target="EditBank{{$bank->id}}"
                                                ...
                                                <a href="#!" data-modal-target="deleteModal{{$bank->id}}"
                                    ...
                                    @include('tenants.finance.bank.delete')
                                    @include('tenants.finance.bank.edit')
                                @endforeach
                            </tbody>
                        </table>
                    </div>
                    <div class="flex flex-col items-center gap-4 px-4 mt-4 md:flex-row" id="pagination-element">
                        <div class="grow">
                            <p class="text-slate-500 dark:text-zink-200">
                                Showing <b class="showing">{{ $banks->firstItem() }}</b> to
                                <b class="showing">{{ $banks->lastItem() }}</b> of
                                <b class="total-records">{{ $banks->total() }}</b> Results
                            </p>
                        </div>
    ...
    @include('tenants.finance.bank.create')
@endsection