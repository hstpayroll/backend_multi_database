@if (session('updating'))
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            var modalId = 'EditsubDepartment{{ session('id') }}';
...
<div id="EditsubDepartment{{$subdepartment->id}}" modal-center class="fixed flex flex-col hidden transition-all duration-300 ease-in-out left-2/4 z-drawer -translate-x-2/4 -translate-y-2/4">
    <div class="w-screen md:w-[30rem] bg-white shadow rounded-md dark:bg-zink-600">
        <div class="flex items-center justify-between p-4 border-b dark:border-zink-500">
            <h5 class="text-16">Edit department {{$subdepartment->id}}</h5>
            <button data-modal-close="EditsubDepartment{{$subdepartment->id}}"
                class="transition-all duration-200 ease-linear text-slate-400 hover:text-red-500">
                <i data-lucide="x" class="w-5 h-5"></i></button>
        </div>

        @if (session('updating') && $subdepartment->id == session('id')) 
        ...
        <div class="max-h-[calc(theme('height.screen')_-_180px)] p-4 overflow-y-auto">
            <form action="{{route('sub_departments.update', $subdepartment->id)}}" method="POST">
                @csrf
                @method('PUT')
                    <div class="grid grid-cols-1 gap-4 xl:grid-cols-12">
                        ...
                        <div class="xl:col-span-12">
                            ...
                            <select name="department_id" id="department_id" class="form-input 
                                @if ($errors->has('department_id') && session('creating') && $subdepartment->id == session('id'))
                                ...
                                @foreach($departments as $department)
                                    <option value="{{ $department->id }}"@if($department->id == optional($subdepartment->department)->id) selected @endif>{{ $department->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="xl:col-span-12">
                            ...
                            class="form-input
                            @if ($errors->has('name') && session('updating') && $subdepartment->id == session('id'))
                            ...
                            @if (session('updating') && $subdepartment->id == session('id')) 
                            value="{{ old('name') }}" placeholder="Department Name"
                            @else
                            value="{{ $subdepartment->name }}" placeholder="Sub-Department Name"
                            @endif>
                ...
                <div class="flex justify-end gap-2 mt-4">
                    <button type="reset" data-modal-close="EditsubDepartment{{$subdepartment->id}}"
