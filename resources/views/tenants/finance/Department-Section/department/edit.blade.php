@if (session('updating'))
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            var modalId = 'EditDepartment{{ session('id') }}';
...
<div id="EditDepartment{{$department->id}}" modal-center class="fixed flex flex-col hidden transition-all duration-300 ease-in-out left-2/4 z-drawer -translate-x-2/4 -translate-y-2/4">
    <div class="w-screen md:w-[30rem] bg-white shadow rounded-md dark:bg-zink-600">
        <div class="flex items-center justify-between p-4 border-b dark:border-zink-500">
            <h5 class="text-16">Edit department {{$department->id}}</h5>
            <button data-modal-close="EditDepartment{{$department->id}}"
                class="transition-all duration-200 ease-linear text-slate-400 hover:text-red-500">
                <i data-lucide="x" class="w-5 h-5"></i></button>
        </div>

        @if (session('updating') && $department->id == session('id')) 
        ...
        <div class="max-h-[calc(theme('height.screen')_-_180px)] p-4 overflow-y-auto">
            <form action="{{route('departments.update', $department->id)}}" method="POST">
                ...
                @method('PUT')
                    <div class="grid grid-cols-1 gap-4 xl:grid-cols-12">
                        ...
                        <div class="xl:col-span-12">
                            ...
                            class="form-input
                            @if ($errors->has('name') && session('updating') && $department->id == session('id'))
                ...
                <div class="flex justify-end gap-2 mt-4">
                    <button type="reset" data-modal-close="EditDepartment{{$department->id}}"
