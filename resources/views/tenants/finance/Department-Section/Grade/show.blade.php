...
@section('department')
    <div class="px-6">
        <div class="flex items-center gap-3 mb-4">
            <h6 class="text-15 grow">Grade</h6>
            <div class="shrink-0">
                <a href="#!" data-modal-target="AddGrade" type="button"
            ...
            <table class="w-full whitespace-nowrap">
                <thead class="ltr:text-left rtl:text-right">
                    <tr>
                        ...
                        <th class="px-3.5 py-2.5 font-semibold border border-slate-200 dark:border-zink-500">Grade Name</th>
                ...
                <tbody class="">
                    @foreach($grades as $grade)
                        <tr>
                            ...
                            <td class="px-3.5 py-2.5 border border-slate-200 dark:border-zink-500">{{$grade->name}}</td>
                            <td class="px-3.5 py-2.5 border border-slate-200 dark:border-zink-500">
                                <div class="flex gap-2">
                                    <a href="#!" data-modal-target="EditGrade{{$grade->id}}"
                                    ...
                                    <a href="#!" data-modal-target="DeleteGrade{{$grade->id}}"
                        ...
                        @include('tenants.finance.Department-Section.grade.delete')
                        @include('tenants.finance.Department-Section.grade.edit') 
                    @endforeach
                </tbody>
            </table>
        </div>
        <div class="flex flex-col items-center gap-4 px-4 mt-4 md:flex-row" id="pagination-element">
            <div class="grow">
                <p class="text-slate-500 dark:text-zink-200">
                    Showing <b class="showing">{{ $grades->firstItem() }}</b> to
                    <b class="showing">{{ $grades->lastItem() }}</b> of
                    <b class="total-records">{{ $grades->total() }}</b> Results
                </p>
            </div>
        ...
        @include('tenants.finance.Department-Section.grade.create')
    </div>
@endsection